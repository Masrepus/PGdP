...
public class MutableList<T> {
     ...
     */
    public MutableList<T> duplicate() {
        MutableList<T> tmp = new MutableList<>();
    ...
    public boolean equals(Object obj) {
        //frst check the type
        if (!(obj instanceof MutableList)) return false;

        //types match, check the entries
        for (int i = 0; i < length(); i++) {
            if (!(((MutableList)obj).get(i).equals(get(i)))) return false;
        }
